...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        assertion = !(builtins.elem cfg.host [ "0.0.0.0" "::" ]);
        message = ''
          services.remarkable-mcp.host must be a concrete address. Wildcard
          addresses cannot produce a safe FastMCP Host/Origin allowlist.
        '';
      }
    ];
